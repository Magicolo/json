...
#[inline]
fn as_hex(character: u8) -> Option<u32> {
    match character {
...
fn parse_number(stream: &mut Stream, value: f64) -> (Option<u8>, f64) {
    let mut value = value;
    while let Some(character) = stream.next() {
        match character {
            b'0'..=b'9' => value = value * 10.0 + (character - b'0') as f64,
            b'.' => return parse_fraction(stream, value),
            b'e' | b'E' => return parse_exponent(stream, value),
            _ => return (Some(character), value),
        }
    }
    (None, value)
}

fn parse_fraction(stream: &mut Stream, value: f64) -> (Option<u8>, f64) {
    let mut fraction = 0.0;
    let mut digits = 0;
    while let Some(character) = stream.next() {
        if let Some(number) = as_number(character) {
            fraction = fraction * 10.0 + number as f64;
            digits += 1;
        } else if character == b'e' || character == b'E' {
            return parse_exponent(stream, value + fraction / POSITIVES[digits]);
        } else {
            return (Some(character), value + fraction / POSITIVES[digits]);
        }
    }
    (None, value + fraction / POSITIVES[digits])
}

fn parse_exponent(stream: &mut Stream, value: f64) -> (Option<u8>, f64) {
    let mut powers = &POSITIVES;
    let mut exponent: usize = 0;

    while let Some(character) = stream.next() {
        if let Some(number) = as_number(character) {
            exponent = exponent * 10 + number as usize;
        } else if character == b'-' {
            powers = &NEGATIVES;
        } else if character == b'+' {
        } else {
            return (Some(character), value * powers[exponent]);
        }
    }
    (None, value * powers[exponent])
}
...
pub fn parse(json: &str) -> Result<Tree, String> {
    ...
    let mut stream = Stream {
        index: 0,
        count: json.len(),
        characters: &json.as_bytes(),
    };
    while let Some(mut character) = stream.next() {
        // deal with numbers first such that no look-ahead is required
        match character {
            b'-' => {
                if let Some(current) = stream.next() {
                    match current {
                        b'0'..=b'9' => {
                            let (next, value) = parse_number(&mut stream, (current - b'0') as f64);
            ...
            b'0'..=b'9' => {
                let (next, value) = parse_number(&mut stream, (character - b'0') as f64);
                stack.push(Node::Number(value));
        ...
        match character {
            b'n' | b'N' => {
                if let (Some(b'u'), Some(b'l'), Some(b'l')) =
            ...
            b'f' | b'F' => {
                if let (Some(b'a'), Some(b'l'), Some(b's'), Some(b'e')) =
            ...
            b't' | b'T' => {
                if let (Some(b'r'), Some(b'u'), Some(b'e')) =
                    (stream.next(), stream.next(), stream.next())
                {
                                    ...
                                    match character {
                                        ...
                                        b'u' => {
                                            if let (
                                                ...
                                                stream.next().and_then(as_hex),
                                                stream.next().and_then(as_hex),
                                                stream.next().and_then(as_hex),
                                                stream.next().and_then(as_hex),
                                            ) {
                                                let hex = hex0 << 12 | hex1 << 8 | hex2 << 4 | hex3;
